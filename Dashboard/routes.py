...
def amplifier_power(mode):
    try:
        if mode == "ON":
            ...
            # GPIO 76 1 AMP ON
            os.system('sudo gpioset 1 76=1')
        elif mode == "OFF":
            # GPIO 76 0 AMP OFF
            os.system('sudo gpioset 1 76=0')
    except Exception as error:
...
def extension_power_controller(mode):
    ...
    try:
        if mode == "ON":
            ...
            os.system('sudo gpioset 1 80=1')
        elif mode == "OFF":
            # GPIO 80 0 extension_power OFF
            os.system('sudo gpioset 1 80=0')
    except Exception as error:
        send_statistic('ACTIVE_UPDATE', 'power_supply_amp_ Error ' + str(error))
...
def tube1tube2_controller(mode):
    ...
    try:
        if mode == "ON":
            ...
            os.system('sudo gpioset 1 79=0')
            os.system('sudo gpioset 1 89=0')
        elif mode == "OFF":
            # GPIO 79+89 1 TUBE OFF
            os.system('sudo gpioset 1 79=1')
            os.system('sudo gpioset 1 89=1')
    except Exception as error:
